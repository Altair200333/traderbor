...
class DecisionProviderTests(unittest.TestCase):
    ...
    def test_deterministic_exchange_replay_prompt_uses_runner_scan(self) -> None:
        ...
        self.assertIn("not veto reasons", prompt)
        self.assertIn("never tighter than the measured noise floor", prompt)
        self.assertIn("A fresh breakout needs no retest confirmation to be tradable", prompt)
    ...
    def test_codex_provider_deterministic_prompt_and_mcp_env(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_run(command, input, text, encoding, errors, capture_output, timeout, env, cwd):
                ...
                self.assertIn("no retest confirmation is required for entry", input)
                self.assertIn("never tighter than the noise% floor from the scan row", input)
