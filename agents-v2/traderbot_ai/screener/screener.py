...
def _scan_symbol(symbol: str, as_of_ms: int, cfg: ScreenerConfig, state: TradingState, cache_path: Path, funding: dict[str, float], btc_roc_4h: float | None) -> SymbolRow:
    ...
    side_rows: dict[str, tuple[dict[str, GateResult], list[PatternHit], PlanPrimitives | None]] = {}
    for side in ("long", "short"):
        ...
        plan = build_plan_primitives(side, side_patterns, frame, atr_value, cfg) if atr_value is not None and gates_pass(gates) else None
        side_rows[side] = (gates, side_patterns, plan)
    signal_side = _choose_signal_side(side_rows)
    candidate = None
    blocked_by: list[str] = []
    plan = None
    if signal_side is not None:
        per_symbol_blocks, global_blocks = state_blocks(normalized, signal_side, state, as_of_ms, cfg)
...
def _choose_signal_side(side_rows: dict[str, tuple[dict[str, GateResult], list[PatternHit], PlanPrimitives | None]]) -> CandidateSide | None:
    passed: list[CandidateSide] = []
    for side in ("long", "short"):
        if gates_pass(side_rows[side][0]) and side_rows[side][2] is not None:
            passed.append(side)
    if len(passed) == 1:
        return passed[0]
    if len(passed) == 2:
...
def _least_failed_side(side_rows: dict[str, tuple[dict[str, GateResult], list[PatternHit], PlanPrimitives | None]]) -> CandidateSide:
