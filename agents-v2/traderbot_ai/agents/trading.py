...
- Use get_setup_digest support/resistance facts before entry. Levels are sorted nearest-first with touches, age_bars, and timeframe; for longs, cite the first support level; for shorts, the first resistance level. Respect the first opposing level on the TP side (resistance above for longs, support below for shorts). For shorts this is a hard rule: the first support below is where bounces start; set TP in front of that support, and if RR then falls below 1.5, hold. For longs overhead resistance is context, not a veto: a fresh breakout is expected to attack overhead levels; prefer TP near 3R and hold only if the first heavily-tested resistance sits closer than 2R. The digest provides no recommended stop, bars, or noise metrics: derive the stop from structural invalidation and typical bar noise (ATR and recent 1h candles you fetch yourself). Enter only if the chosen stop is feasible, risk budget holds, and RR targets 3.0 (never below 2.0) at a realistic TP. Otherwise hold. Never tighten the stop to fit risk.
...
- Deterministic candidates are pre-selected fresh momentum breakouts: extension, a large last candle, last-hour impulse, or high RSI are expected properties of this flow, not veto reasons, and no retest confirmation is required for entry. Hold a candidate only for concrete operational reasons: data warnings on its row, stale or implausible price, entry drift beyond the runner clamp, documented illiquidity, or exhausted risk budget / position slots. On multi-candidate bars open at most the free position slots, prefer the more liquid symbol on close calls, and do not pick solely by the largest recent ROC.
...
- Stop distance must be the larger of 2.0 * ATR(14, 1h) as a fraction of price, the measured noise floor (noise% column in the scan table; the runner rejects entries with a tighter stop), and structural invalidation distance beyond the broken boundary or nearest support/resistance with a 0.5 * ATR buffer.
- Use get_setup_digest support/resistance facts before entry. Levels are sorted nearest-first with touches, age_bars, and timeframe; for longs, cite the first support level; for shorts, the first resistance level. Respect the first opposing level on the TP side (resistance above for longs, support below for shorts). For shorts this is a hard rule: the first support below is where bounces start; set TP in front of that support, and if RR then falls below 1.5, hold. For longs overhead resistance is context, not a veto: a fresh breakout is expected to attack overhead levels; prefer TP near 3R and hold only if the first heavily-tested resistance sits closer than 2R. Enter only if the chosen stop is feasible, risk budget holds, and RR targets 3.0 (never below 2.0) at a realistic TP. Otherwise hold. Never tighten the stop to fit risk.
- retest_seen counts only closed bars after the breakout trigger bar. retest_seen=false with trigger_age_bars=0 means a retest is not possible yet, not that a retest failed.
- Shorts are not mirrored longs: crypto downside moves are impulsive and mean-revert fast. Do not short an exhausted move: prefer fresh breakdowns confirmed by retest_seen=true, avoid short entries with RSI near the oversold bound or price extended more than 1.5 ATR below EMA20(1h), and require the TP to be reachable before the first support below.
- Stop distance must be in [1.0%, 4.0%] after tick rounding. If the structural stop needs more than 4.0%, return hold; do not tighten the stop to fit.
- Reward:risk must be >= 1.5 after rounding.
